...
def generate_new_ann(
    ...
    if not dataFound:
        ...
        name_ = None
        if args.dataset == "dl_test_2019": # If development set is 2020 test dataset; then generate embeddings for 2019 test dataset stored originally as dev-query
        ...
        if is_first_worker():
            print("dev_query_embedding size: ", dev_query_embedding.shape)
        ...
        if args.dataset == "dl_test_2019":
            ...
            raise Exception('Dataset should be one of {dl_test_2020, dl_test_2019, dl_test_2019_1, dl_test_2019_2}!!')
        dev_query_cache = EmbeddingCache(dev_query_collection_path)
        with dev_query_cache as emb:
            dev_query_embedding, dev_query_embedding2id = StreamInferenceDoc(args, model, GetProcessingFn(
                args, query=True), name_ + str(latest_step_num) + "_", emb, is_query_inference=True)
