...
def GetTripletTrainingDataProcessingFn(args, query_cache, passage_cache):
    def fn(line, i):
        line_arr = line.split('\t')
        qid = int(line_arr[0])
        pos_data = line_arr[1].split(":")
        pos_pid = int(pos_data[0])
        pos_score = float(pos_data[1])
        ...
        for idx, neg_pid in enumerate(neg_pids):
            neg_data = GetProcessingFn(
                args, query=False)(
                passage_cache[neg_pid], neg_pid)[0]
            yield (query_data[0], query_data[1], query_data[2], pos_data[0], pos_data[1], pos_data[2],
                   neg_data[0], neg_data[1], neg_data[2], pos_score, neg_scores[idx])
