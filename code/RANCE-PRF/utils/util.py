...
class BM25_helper:
    ...
    def __init__(self, args):
        self.idx2q_t, self.idx2q_t1, self.idx2q_t2 = self.get_idx2query_map(args)
        self.tok2df = self.get_df_map(args)
        self.tok2tf = self.get_tf_map(args)
        self.N, self.avgl, self.docidx2len = self.get_corpus_stats(args)
        self.anceidx2gidx, self.idx2pid = self.get_anceidx2gidx(args)

    def get_anceidx2gidx(self, args):
        f = open(args.freqmap_path + '/doc_id2gidx.pickle', 'rb')
        pid2gidx = pickle.load(f)
        f.close()

        idx2pid = {}

        f_docs = open(args.id2offset_path + '/pid2offset.pickle', 'rb')
        pid2idx = pickle.load(f_docs)
        idx2pid = {v: k for k, v in pid2idx.items()}
        anceidx2gidx = {v: pid2gidx[k] for k, v in pid2idx.items()}
        f_docs.close()

        return anceidx2gidx, idx2pid


    def get_pid2gidx_map(self, args):
        f = open(args.freqmap_path + '/doc_id2gidx.pickle', 'rb')
        pid2gidx = pickle.load(f)
        f.close()
    ...
    def get_corpus_stats(self, args):

        f = open(args.freqmap_path + '/docs_length.pickle', 'rb')
        docidx2len = pickle.load(f)
        f.close()
        N = len(docidx2len.keys())
        num_docs = 0
        tot_len = 0
        for k, v in docidx2len.items():
            num_docs += 1
            tot_len += v
        assert N == num_docs
        avgl = (tot_len * 1.0) / num_docs

        return N, avgl, docidx2len

    def get_tf_map(self, args):

        f = open(args.freqmap_path + '/tf_dic.pickle', 'rb')
        tf_map = pickle.load(f)
        f.close()

        return tf_map

    def get_df_map(self, args):

        f = open(args.freqmap_path + '/df_dic.pickle', 'rb')
        df_map = pickle.load(f)
        f.close()

        return df_map

    def get_idx2query_map(self, args):

        idx2q_t = {}
        idx2q_t1 = {}
        idx2q_t2 = {}

        f_train = open(args.id2offset_path + '/train_qid2offset.pickle', 'rb')
        qid2idx_train = pickle.load(f_train)
        f_train.close()

        f_dev = open(args.id2offset_path + '/test_1_qid2offset.pickle', 'rb')
        qid2idx_test1 = pickle.load(f_dev)
        f_dev.close()

        f_dev = open(args.id2offset_path + '/test_2_qid2offset.pickle', 'rb')
        qid2idx_test2 = pickle.load(f_dev)
        f_dev.close()

        with open(args.raw_data_path + '/msmarco-doctrain-queries.tsv') as f:
            for line in f:
                (key, value) = line.split('\t')
                value = value.translate(value.maketrans('', '', string.punctuation))
                try:
                    idx = qid2idx_train[int(key)]
                except KeyError:
                    continue
                idx2q_t[int(idx)] = value

        with open(args.raw_data_path + '/msmarco-test2019-queries_1.tsv') as f:
            for line in f:
                (key, value) = line.split('\t')
                value = value.translate(value.maketrans('', '', string.punctuation))
                try:
                    idx = qid2idx_test1[int(key)]
                except KeyError:
                    continue
                idx2q_t1[int(idx)] = value

        with open(args.raw_data_path + '/msmarco-test2019-queries_2.tsv') as f:
            for line in f:
                (key, value) = line.split('\t')
                value = value.translate(value.maketrans('', '', string.punctuation))
                try:
                    idx = qid2idx_test2[int(key)]
                except KeyError:
                    continue
                idx2q_t2[int(idx)] = value

        return idx2q_t, idx2q_t1, idx2q_t2
...
def compute_BM25_score(n, f, qf, r, N, dl, avdl):
    K = compute_K(dl, avdl)
